...
type GSSAPIClient interface {
	...
	InitSecContext(targetName string, token []byte) (outputToken []byte, needContinue bool, err error)
...
func (m *GSSAuthMethod) RunAuth(conn net.Conn, pool BufferPool) (net.Conn, AuthInfo, error) {
	...
	inputToken := []byte(nil)
	for {
		outToken, needContinue, err := m.Client.InitSecContext(m.TargetName, inputToken)
		if err != nil {
		...
		if hdr[0] != 0x01 || hdr[1] != 0x01 {
			return conn, info, fmt.Errorf("invalid GSS frame: ver=0x%x mtyp=0x%x", hdr[0], hdr[1])
		}
...
func (h *GSSAuthHandler) HandleAuth(conn net.Conn, pool BufferPool) (net.Conn, AuthInfo, error) {
	...
	for {
		...
		outToken, name, needContinue, err := h.Server.AcceptSecContext(clientToken)
